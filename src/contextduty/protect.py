...
from ._workspace import load_policy_auto, scan_workspace
from .adapters.ide import AI_TOOLS, AITool, write_all_ignore_files  # noqa: F401 (re-export)
...
__all__ = [
    ...
    "protect_watch",
    "protect_status",
    # Re-exported so existing callers of contextduty.protect.AI_TOOLS still work.
    "AI_TOOLS",
    "AITool",
]
...
def protect_workspace(
    ...
    policy = load_policy_auto(policy_path)
    ...
    _print_findings(sensitive_files)

    written = write_all_ignore_files(out_dir, sensitive_files)
    _print_written(written, out_dir)
    _print_coverage_status(workspace)
...
def protect_watch(
    ...
    """Watch *workspace* and update ALL ignore files when sensitive files change."""
    policy = load_policy_auto(policy_path)
    ...
    try:
        while True:
            ...
            if current_state != last_state:
                ...
                write_all_ignore_files(workspace, sensitive_files)
                _log_watch_changes(added, removed, current_state, last_state)
...
def protect_status(workspace: Path) -> int:
    ...
    _print_header("Protection Status")
    _print_coverage_status(workspace)
...
# ─────────────────────────────────────────────────────────────────────────────
# Private rendering helpers — output only, no business logic
# ─────────────────────────────────────────────────────────────────────────────


def _print_header(subtitle: str = "Universal AI Workspace Protection") -> None:
    print(f"\n{style.bold}{'─' * 56}{style.reset}")
    print(f"{style.bold}  ContextDuty — {subtitle}{style.reset}")
    print(f"{style.bold}{'─' * 56}{style.reset}\n")


def _print_findings(sensitive_files: list[tuple[str, set[str]]]) -> None:
    print(
        f"  {style.yellow}⚠{style.reset}  "
        f"{style.bold}{len(sensitive_files)}{style.reset} file(s) contain secrets/PII:\n"
    )
...
def _print_written(written: list[Path], out_dir: Path) -> None:
    print(
        f"  {style.green}✓{style.reset}  Written "
        f"{style.bold}{len(written)}{style.reset} ignore files:\n"
    )
    ...
    for tool in AI_TOOLS:
        ...
        if ignore_path.exists():
            ...
            print(
                ...
                f"{style.dim}{len(entries)} files blocked{style.reset}"
            )
        else:
            print(
                f"  {style.red}✗{style.reset}  {tool.name:<25} "
                f"{style.dim}not configured{style.reset}"
            )


def _log_watch_changes(
    added: set[str],
    removed: set[str],
